# 脚本命令参考

> 单一数据源：`package.json`

本文档列出所有可用的 npm scripts，基于 `package.json` 自动生成。

## 目录

- [开发模式](#开发模式)
- [生产构建](#生产构建)
- [代码质量](#代码质量)
- [项目信息](#项目信息)

---

## 开发模式

### H5 开发

```bash
pnpm dev
# 或
pnpm dev:h5
```

启动 H5 开发服务器，支持热更新。

**端口：** 默认 5173（可通过 Vite 配置修改）

**特点：**
- ⚡️ Vite HMR 热更新
- 📦 自动导入组件和 API
- 🎨 UnoCSS 即时编译
- 🔍 TypeScript 类型检查

### Android 开发

```bash
pnpm dev:android
# 或
pnpm dev:app
```

启动 Android App 开发模式。

**前置条件：**
- 安装 HBuilderX 或 Android Studio
- 连接 Android 设备或启动模拟器
- 配置 `manifest.config.ts` 中的 `appid`

**产物位置：** `unpackage/dist/dev/app-plus/`

### HarmonyOS 开发

```bash
pnpm dev:harmony
```

启动 HarmonyOS App 开发模式。

**前置条件：**
- HBuilderX 4.27+
- DevEco Studio 5.0+
- HarmonyOS 真机或模拟器

**产物位置：** `unpackage/dist/dev/app-harmony/`

---

## 生产构建

### H5 构建

```bash
pnpm build
# 或
pnpm build:h5
```

构建 H5 生产版本。

**产物位置：** `unpackage/dist/build/h5/`

**优化项：**
- 代码压缩和混淆
- Tree-shaking 去除未使用代码
- 资源哈希化（长期缓存）
- 生成 source map（可选）

### Android 构建

```bash
pnpm build:android
# 或
pnpm build:app
```

生成 Android 打包资源。

**产物位置：** `unpackage/resources/`

**注意：** 此步骤仅生成资源，实际 APK 需要通过以下方式完成：
- **云打包**：使用 HBuilderX 云打包服务
- **本地打包**：使用 Android Studio 打包

详细步骤见 [BUILD.md](./BUILD.md#android-apk-打包)

### HarmonyOS 构建

```bash
pnpm build:harmony
```

生成 HarmonyOS 工程文件。

**产物位置：** `unpackage/dist/build/app-harmony/`

**注意：** 实际 HAP 包需要使用 DevEco Studio 打包。

详细步骤见 [BUILD.md](./BUILD.md#harmonyos-应用打包)

---

## 代码质量

### TypeScript 类型检查

```bash
pnpm type-check
```

执行 TypeScript 编译器进行类型检查。

**检查项：**
- 类型注解完整性
- 接口兼容性
- 泛型使用正确性
- 严格模式违规

**不生成输出文件**（`--noEmit`）

### ESLint 代码检查

```bash
pnpm lint
```

执行 ESLint 检查并自动修复可修复的问题。

**配置文件：** `eslint.config.js`

**功能：**
- ✅ 自动修复代码格式问题
- ✅ 删除未使用的导入
- ✅ 排序导入语句
- ✅ 统一引号和缩进
- ⚠️ 部分复杂问题需要手动修复

---

## 项目信息

### 查看项目信息

```bash
pnpm about
```

显示项目详细信息和统计数据。

**输出内容：**
- 项目名称和版本
- 依赖树大小
- 构建工具版本
- uni-app 版本信息

---

## Git Hooks

### Pre-commit Hook

```bash
# 自动触发（无需手动运行）
pnpm lint-staged
```

每次提交前自动运行，由 `simple-git-hooks` 配置。

**执行流程：**
1. 触发 Git pre-commit hook
2. 运行 `lint-staged`
3. 对暂存文件执行 `eslint --fix`
4. 检查通过后允许提交
5. 检查失败阻止提交

**配置位置：** `package.json` → `lint-staged`

### Post-install Hook

```bash
# 自动触发（无需手动运行）
npx simple-git-hooks
```

安装依赖后自动设置 Git hooks。

---

## 平台对比

| 平台 | 开发命令 | 构建命令 | 产物位置 | 最终打包 |
|------|---------|---------|---------|---------|
| **H5** | `pnpm dev` | `pnpm build` | `unpackage/dist/build/h5/` | 直接部署 |
| **Android** | `pnpm dev:android` | `pnpm build:android` | `unpackage/resources/` | Android Studio / 云打包 |
| **HarmonyOS** | `pnpm dev:harmony` | `pnpm build:harmony` | `unpackage/dist/build/app-harmony/` | DevEco Studio |

---

## 环境要求

**必需版本：**
- Node.js: ≥24.0.0
- pnpm: ≥10

**详细安装步骤：** @GETTING_STARTED.md#环境要求

---

## 常见问题

### Q: `pnpm dev` 启动失败？
A: 检查 Node.js 版本，清理 node_modules 重新安装。详见 @GETTING_STARTED.md#常见问题

### Q: `pnpm build` 报类型错误？
A: 运行 `pnpm type-check` 查看详细错误。详见 @RUNBOOK.md#常见问题

### Q: Android/HarmonyOS 构建后没有安装包？
A: 构建命令只生成资源，需要使用对应工具完成最终打包。详见 @BUILD.md

### Q: Git hook 不生效？
A: 重新安装依赖触发 post-install hook。详见 @GETTING_STARTED.md#常见问题

---

## 相关文档

- [构建指南](./BUILD.md) - 详细的构建和打包流程
- [环境配置](./ENV.md) - 环境变量配置
- [开发工作流](./CONTRIB.md) - 开发规范和最佳实践

---

最后更新：2026-02-04
数据源：package.json
