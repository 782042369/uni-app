...
router.beforeEach((to, from, next) => {
  console.log('[Router] beforeEach 守卫触发:', { to, from })
  ...
  if (to.name === 'demo-protected') {
    ...
    return new Promise<void>((resolve, reject) => {
      showConfirm({
        ...
        cancelButtonText: '取消',
        success() {
          console.log('[Router] 用户确认访问，允许导航')
          next()
          resolve()
        },
        fail() {
          console.log('[Router] 用户取消访问，阻止导航')
          next(false)
...
router.afterEach((to, from) => {
  console.log('[Router] afterEach 钩子触发:', { to, from })

  // 演示：简单的页面切换记录
  if (to.path) {
    console.log(`[Router] 页面切换完成: ${to.path}`)
  }

  // 演示：针对 afterEach 演示页面的简单提示
  if (to.name === 'demo-aftereach') {
    const { show: showToast } = useGlobalToast()
    console.log('[Router] 进入 afterEach 演示页面')
    setTimeout(() => {
